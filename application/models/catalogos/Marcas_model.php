...
class Marcas_model extends CI_Model{
    ...
    public function cargar(){
        $query = $this->db->get_where('marcas', array('eliminado' => 0));
        return $query->result();
    }

    public function agregar($datos, $nom_marca){
        $row = $this->db->get_where('marcas', array('eliminado' => 0, 'nom_marca' => $nom_marca));
        if($row->num_rows() == 0){
    ...
    public function eliminar($id){
        $this->db->where('id', $id);
        $this->db->update('marcas', array('eliminado' => 1));
